...
BinTree<int> reconstruir(vector<int> const& preorden, vector<int> const& inorden) {
    if (preorden.empty()) return BinTree<int>();
    if (preorden.size() == 1) return BinTree<int>(preorden[0]);

    int raiz = preorden[0];

    int i = 0;
    while (i < inorden.size() && inorden[i] != raiz) {
        i++;
    }

    vector<int> preordenIzdo, preordenDcho;
    for (int j = 1; j <= i; j++) {
        preordenIzdo.push_back(preorden[j]);
    }
    for (int j = i + 1; j < preorden.size(); j++) {
        preordenDcho.push_back(preorden[j]);
    }
    vector<int> inordenIzdo, inordenDcho;
    for (int j = 0; j < i; j++)
        inordenIzdo.push_back(inorden[j]);
    for (int j = i + 1; j < inorden.size(); j++)
        inordenDcho.push_back(inorden[j]);

    BinTree<int> resul(reconstruir(preordenIzdo, inordenIzdo), preorden[0], reconstruir(preordenDcho, inordenDcho));
    return resul;
}
...
bool casoDePrueba() {
    ...
    vector<int> preorden(numNodos);
    vector<int> inorden(numNodos);
