...
pair<int, bool> completo(const BinTree<char>& arbol) {
    if (arbol.empty())
        return { 0, true };
        ...
        if (!arbol.left().empty() && !arbol.right().empty()) {
            pair<int, bool> aux1 = completo(arbol.left());
            pair<int, bool> aux2 = completo(arbol.right());

            pair<int, bool> result;

            result.second = aux1.second && aux2.second && (aux1.first == aux2.first);
            result.first = aux1.first + 1; //da igual entre poner aux1 o aux2 pq si second es true, son iguales, y, si no, es indiferente el nivel porque ya no va a ser completo
            return result;
        }
        else {
            return { -1, false };
