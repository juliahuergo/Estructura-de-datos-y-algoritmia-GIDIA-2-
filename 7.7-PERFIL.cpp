...
template <typename T>
void listaPerfil(const BinTree<T>& arbol, int nivelActual, int& nivelMax, vector<T>& res) {
    if (!arbol.empty()) {
        if (nivelActual > nivelMax) {
            res.push_back(arbol.root());
            nivelMax = nivelActual;
        }
        listaPerfil(arbol.left(), nivelActual+1, nivelMax, res);
        listaPerfil(arbol.right(), nivelActual+1, nivelMax, res);
    }
}

template <typename T> 
vector <T> perfil(const BinTree<T>& arbol) {
    vector<T> res;

    int nivelMax = 0;
    listaPerfil(arbol, 1, nivelMax, res);
    return res;
}
...
void casoDePrueba() {
    ...
    vector <int> resul = perfil<int>(arbol);
