...
template <typename T>
void recorInorden(const BinTree<T>& arbol, vector<T>& ans) {
    if (!arbol.empty()) {
        if (arbol.left().empty() && arbol.right().empty()) {
            ans.push_back(arbol.root());
        }
        else {
            recorInorden(arbol.left(), ans);
            ans.push_back(arbol.root());
            recorInorden(arbol.right(), ans);
        }
    }
}
...
bool casoDePrueba() {
    ...
    if (tipo == 'N') {
        ...
        vector<int> resul = {};
        recorInorden<int>(arbol, resul); 
        
        if (resul.size() > 1) {
            int i = 0;
            while (i < resul.size() - 1 && esBusqueda) {
        ...
        BinTree<string> arbol = read_tree<string>(std::cin);
        vector<string> resul = {};
        recorInorden<string>(arbol, resul);
        if (resul.size() > 1) {
            int i = 0;
            while (i < resul.size() - 1 && esBusqueda) {
                if (resul[i + 1] <= resul[i])
