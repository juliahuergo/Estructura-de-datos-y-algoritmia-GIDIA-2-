...
template <typename T> BinTree<T> read_treeAguas(std::istream& in) {
    T c;
    if (!(in >> c)) return BinTree<T>(); //si se acaba la entrada
    ...
    if (c == -1) {
        ...
        BinTree<T> left = read_treeAguas<T>(in);  
        BinTree<T> right = read_treeAguas<T>(in);  
        return BinTree<T>(left, c, right);
    }
}

template <typename T> 
int aguas(const BinTree<T>& arbol, int& cont, bool esRaiz) {
    if (arbol.empty())
        return 0;
    else if (arbol.left().empty() && arbol.right().empty())
        return 1;
    else {
        int caudalL = aguas(arbol.left(), cont, false);
        int caudalR = aguas(arbol.right(), cont, false);

        int resul = caudalL + caudalR - arbol.root();
        if (resul < 0) resul = 0;
...
void casoDePrueba() { 
    BinTree<int> arbol = read_treeAguas<int>(std::cin); 

    int cont = 0;
    aguas(arbol, cont, true);
    cout << cont << "\n";
}
