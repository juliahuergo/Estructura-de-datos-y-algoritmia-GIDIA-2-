...
template <typename T>
pair <int, int> diametro(const BinTree<T>& arbol) {
    //se devuelve altura y diámetro
    if (arbol.empty()) 
        return { 0, 0 };
    else {
        auto [alturaL, diamL] = diametro(arbol.left()); 
        auto [alturaR, diamR] = diametro(arbol.right()); 

        pair<int, int> resul;
        resul.first = max(alturaL, alturaR) + 1;
        resul.second = max(max(diamL, diamR), alturaL+ alturaR + 1); //max (todo en un lado, pasando por raíz) 
        return resul;
    }
}
...
void casoDePrueba() {
    ...
    cout << max(diametro<char>(arbol).first, diametro<char>(arbol).second) << "\n";
