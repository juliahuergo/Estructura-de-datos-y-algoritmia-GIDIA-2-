...
/*template < typename T>
std::pair<bool, int> es_balanceado_eficiente(const BinTree<T> tree) {

    if (tree.empty())
        return { true, 0 };

    else {
        auto [es_b_iz, alt_iz] = es_balanceado_eficiente(tree.left());
        auto [es_b_der, alt_der] = es_balanceado_eficiente(tree.right());

        std::pair<bool, int> result;
        result.first = es_b_iz && es_b_der && abs(alt_iz - alt_der) <= 1;

        result.second = 1 + max(alt_iz, alt_der);
        return result;
    }
}*/
...
template <typename T> 
std::pair<int, int> excursionistas(BinTree<T> tree) {
    if (tree.empty()) { //vacío u hoja
        return { 0, 0 };
    }
    else if (tree.left().empty() && tree.right().empty() && tree.root() !=0) {
        return { 1, tree.root()};
    }

    else {
        auto [caminos_izda, max_izda] = excursionistas(tree.left());
        auto [caminos_dcha, max_dcha] = excursionistas(tree.right());
        int num_equipos = caminos_izda + caminos_dcha;
        if (num_equipos == 0 && tree.root() != 0)
            num_equipos++;
        return { num_equipos, tree.root() + max(max_izda, max_dcha) };
    }
}
...
void casoDePrueba() {
    ...
    pair<int, int> result = excursionistas(arbol);
