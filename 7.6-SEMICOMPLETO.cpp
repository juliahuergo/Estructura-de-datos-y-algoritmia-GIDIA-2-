...
tuple<int, bool, bool> completo(const BinTree<char>& arbol) { //altura, completo, semicompleto
    if (arbol.empty())
        return { 0, true, true };
    else if (arbol.left().empty() && arbol.right().empty())
        return { 1, true, true };
    else {
            tuple<int, bool, bool> aux1 = completo(arbol.left());
            tuple<int, bool, bool> aux2 = completo(arbol.right());
            //auto [alt_iz, compl_iz, semicom_iz ]

            tuple<int, bool, bool> result;

            get<1>(result) = (!arbol.left().empty() && !arbol.right().empty()) && get<1>(aux1) && get<1>(aux2) && (get<0>(aux1) == get<0>(aux2)); //COMPLETO

            if (get<1>(result))
                get<2>(result) = true; //TODO ÁRBOL COMPLETO ES SEMICOMPLETO
            else {
                get<2>(result) = ((get<1>(aux1) && get<2>(aux2) && (get<0>(aux1) == get<0>(aux2))) || (get<2>(aux1) && get<1>(aux2) && (get<0>(aux1) == get<0>(aux2) + 1)) || (get<1>(aux1) && get<1>(aux2)) && (get<0>(aux1) == get<0>(aux2) + 1));
            }

            get<0>(result) = max(get<0>(aux1), get<0>(aux2)) + 1; //da igual entre poner aux1 o aux2 pq si second es true, son iguales, y, si no, es indiferente el nivel porque ya no va a ser completo
            return result;
    }
...
void casoDePrueba() {
    ...
    if (get<1>(completo(arbol)))
        cout << "COMPLETO\n";
    else if (get<2>(completo(arbol)))
        cout << "SEMICOMPLETO\n";
    else
        cout << "NADA\n";
