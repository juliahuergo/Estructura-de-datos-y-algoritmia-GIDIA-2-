...
//int es el nivel en el que están
/*pair<int, bool> completo2(const BinTree<char>& arbol, int& cont) {
    if (arbol.empty()) {
        cont = max(cont, 0);
        return { 0, true };
    }        
        ...
        if (!arbol.left().empty() && !arbol.right().empty()) {
            auto[nivelI, completoI] = completo(arbol.left(), cont);
            auto[nivelD, completoD] = completo(arbol.right(), cont);

            pair<int, bool> result;
            if (!completoI && !completoD) {
                result = { 1, true };
            }
            else {
                result = { min(nivelI, nivelD) + 1, true };
            }
            cont = max(cont, result.first);
            return result;
        }
        else {
            cont = max(cont, 1);
...
int completo(const BinTree<char>& arbol, int& cont) {
    if (arbol.empty()) {
        cont = max(cont, 0);
        return 0;
    }
        ...
        int nivelI = completo(arbol.left(), cont);
        int nivelD = completo(arbol.right(), cont);

        int result = min(nivelI, nivelD) + 1;
            
        cont = max(cont, result);
        return result;
    }
}
