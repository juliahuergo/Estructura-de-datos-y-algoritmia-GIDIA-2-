...
public:
    ...
    pair<bool, T> lower_bound(T const& e) const {
        return lower_bound(root_node, e); 
    }
...
private:
    ...
    static int count_interval(Node* root, T const& lower, T const& upper) {
        if (root == nullptr)
            ...
            if (root->elem < lower)
                return count_interval(root->right, lower, upper);
            else if (root->elem > upper)
                return count_interval(root->left, lower, upper);
            else {
                if (root->elem == lower)
                    return 1 + count_interval(root->right, lower, upper);
                else if (root->elem == upper)
                    return 1 + count_interval(root->left, lower, upper);
                else
                    return 1 + count_interval(root->right, lower, upper) + count_interval(root->left, lower, upper);
            }
        }
    }

    static pair<bool, T> lower_bound(Node* root, T const& e) {
        //true si hay elems >= que e
        ...
        if (root == nullptr)
            ...
            pair<bool, T> resul = lower_bound(root->left, e);
            if(resul.second >= e)
                return { true, resul.second };
            else
                return { true, root->elem };
        }

        else { //root->elem < e
            pair<bool, T> resul = lower_bound(root->right, e);
            return { resul.second >= e, resul.second };
        }
    }
};
...
bool casoDePrueba() {
    ...
    for (int i = 0; i < M; i++) {
        cin >> pregunta;
        pair<bool, int> resul = arbol.lower_bound(pregunta);
        if (resul.first)
            cout << resul.second << "\n";
        else
            cout << "NO HAY\n";
