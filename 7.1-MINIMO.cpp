...
T minimo(const BinTree <T>& arbol) {
    ...
    if (arbol.left().empty() && arbol.right().empty()) {
        ...
        if (!arbol.left().empty() && !arbol.right().empty()) {
            auto minIzda = minimo(arbol.left());
            auto minDcha = minimo(arbol.right());
            return min({ minIzda, minDcha, arbol.root() });
        }
        else if (!arbol.right().empty()) {
            auto minDcha = minimo(arbol.right()); 
            return min(minDcha, arbol.root());
        }
        else {
            auto minIzda = minimo(arbol.left());
            return min(minIzda, arbol.root());  
        }
    }
}
...
bool casoDePrueba() {
    ...
    if (tipo == 'N') {
        ...
        cout << minimo<int>(arbol) << endl;
    }
    else {
        BinTree<string> arbol = read_tree<string>(std::cin);
        cout << minimo<string>(arbol) << endl;
    }
