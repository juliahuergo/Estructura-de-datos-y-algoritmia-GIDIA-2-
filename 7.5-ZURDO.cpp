...
template <typename T>
std::pair<bool, int> zurdo(BinTree<T> arbol) {
    if (arbol.empty()) { //vacío u hoja
        return { true, 0 };
    }
    else if (arbol.left().empty() && arbol.right().empty())
        return { true, 1 };
    else {
        auto [zurdo_iz, num_iz] = zurdo(arbol.left());
        auto [zurdo_der, num_der] = zurdo(arbol.right());
        pair<bool, int> result;
        result.first = zurdo_iz && zurdo_der && num_iz > num_der;
        result.second = 1 + num_iz + num_der;
        return result;
    }
}
...
void casoDePrueba() {
    ...
    pair<bool, int> result = zurdo<char>(arbol); 

    if (result.first) cout << "SI\n";
    else cout << "NO\n";
}
