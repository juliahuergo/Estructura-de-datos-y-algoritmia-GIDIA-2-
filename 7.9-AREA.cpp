...
template <typename T>
std::pair<int, int> area(const BinTree <T>& arbol) { //primera componente area con el actual, segunda comp. maximo area hasta el momento
    if (arbol.empty()) return { 0,0 };

    else if (arbol.left().empty() && arbol.right().empty()) {
        if (arbol.root() == 'V') return { 1, 1 };
        else return { 0, 0 };
    }
    else {
        auto [maxI, areaActI] = area(arbol.left());
        auto [maxD, areaActD] = area(arbol.right());

        pair<int, int> result;

        if (arbol.root() == 'V') {
            result.second = areaActI + areaActD + 1;
            result.first = max({ result.second, maxI, maxD });
        }

        else {
            result.second = 0;
            result.first = max(maxI, maxD);
        }
        return result;
    }
}
...
void casoDePrueba() {
    ...
    std::cout << area(arbol).first << std::endl;
}
